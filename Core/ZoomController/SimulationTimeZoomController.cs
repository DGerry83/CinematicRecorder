...
namespace CinematicRecorder.Core
{
    ...
    public class SimulationTimeZoomController : MonoBehaviour
    {
        ...
        private void OnPhysicsStepped(float physicsDeltaTime)
        {
            ...
            // If consistent framing is active, apply it (works for both camera types)
            ...
            {
                var ctController = new CameraToolsCameraController(); // Or get from camera if you have a reference
                ctController.ApplyRateStep(_cachedRateInput, physicsDeltaTime);
                return;
            }

            // Normal strategy processing for HullCam or non-rate CT modes
        ...
        public void BeginRateBasedZoom()
        {
            ...
            // Apply current cached input immediately if available
